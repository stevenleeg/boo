...
class Boo:
	def __init__(self):
		self.boofile = os.path.join(os.environ['HOME'], '.boo')
		
		try:
			f = open(self.boofile, 'rb')
			self.data = pickle.load(f)
			f.close()
		except (IOError, EOFError):
			self.data = {}
			self.save()
			print("\033[94m [MEH] \033[0m .boo not found. I'll create it for you...")
	
	def save(self):
		f = open(self.boofile, 'wb')
